...
//!   --mode keyword|hybrid         Search mode (default: keyword)
...
fn load_corpus(path: &Path) -> Corpus {
    let file = std::fs::File::open(path).unwrap_or_else(|e| panic!("Cannot open {}: {}", path.display(), e));
...
fn ndcg_at_k(
    ...
    qrel: &HashMap<String, u32>,
    k: usize,
) -> f64 {
    // Actual DCG from ranked results
    ...
    // Ideal DCG from best possible ranking
...
fn ensure_dataset(data_dir: &Path, name: &str) -> PathBuf {
    ...
    let output = std::process::Command::new("unzip")
        .args(["-q", "-o", zip_path.to_str().unwrap(), "-d", data_dir.to_str().unwrap()])
        .output()
        .expect("Failed to run unzip");
    if !output.status.success() {
        panic!("Failed to unzip: {}", String::from_utf8_lossy(&output.stderr));
    }
...
fn run_dataset(
    ...
    // Enable auto-embed for hybrid mode
    if mode == "hybrid" {
        db.set_auto_embed(true).expect("Failed to enable auto-embed");
    }
    ...
    for (doc_id, doc) in &sorted_docs {
        ...
        db.kv_put(doc_id, Value::String(text.into())).expect("Failed to index doc");
        indexed += 1;
        if indexed % 5000 == 0 {
    ...
    eprintln!("\r  Indexed {} docs in {:.1}s", indexed, index_time.as_secs_f64());

    // Wait for embeddings to complete if hybrid mode
    if mode == "hybrid" {
        ...
        loop {
            ...
            eprint!("\r  Embedding: {}/{} pending", status.pending, status.total_queued);
            std::io::stderr().flush().unwrap();
    ...
    println!();
    println!("============================================================");
    println!("  Dataset: {}  |  Mode: {}", dataset_name, mode);
    println!("  Corpus: {} docs  |  Queries: {}", corpus.docs.len(), eval_queries.len());
    println!("============================================================");
    println!("  nDCG@{}: {:.4}", k, avg_ndcg);
    println!("  Index: {:.1}s  |  Search: {:.1}s  |  QPS: {:.1}",
        index_time.as_secs_f64(), search_time.as_secs_f64(), qps);
...
fn main() {
    ...
    let mut mode = "keyword".to_string();
    let mut k: usize = 10;
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            other => {
                ...
                eprintln!("Usage: beir [--datasets nfcorpus,scifact] [--mode keyword|hybrid] [--k 10] [--data-dir path]");
                std::process::exit(1);
            }
        }
        i += 1;
    ...
    eprintln!("BEIR Benchmark: datasets={:?}, mode={}, k={}", datasets, mode, k);
    eprintln!();

    for name in &datasets {
        let dataset_dir = ensure_dataset(&data_dir, name);
        run_dataset(name, &dataset_dir, &mode, k);
    }
}
