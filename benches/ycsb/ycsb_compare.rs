//! YCSB Competitive Comparison — Strata vs rocksdb, lmdb, sqlite, Redis
//!
...
use workloads::{ycsb_key, FastRng, KeyChooser, Operation, WorkloadSpec, workload_by_label};
...
impl LmdbYcsb {
    fn new(tmpdir: &Path) -> Self {
        ...
        wtxn.commit().unwrap();
        Self { env, db, _dir: dir }
    }
}

impl YcsbDb for LmdbYcsb {
    fn name(&self) -> &'static str {
        "lmdb"
    }
    ...
    fn get(&mut self, key: &str) -> Option<Vec<u8>> {
        ...
        self.db.get(&rtxn, key).unwrap().map(|v| v.to_vec())
    }
...
impl YcsbDb for RedisYcsb {
    fn name(&self) -> &'static str {
        "redis"
    }

    fn put(&mut self, key: &str, value: &[u8]) {
        redis::Commands::set::<_, _, ()>(&mut self.con, key, value).unwrap();
    }

    fn get(&mut self, key: &str) -> Option<Vec<u8>> {
        redis::Commands::get(&mut self.con, key).ok()
    }

    fn scan(&mut self, _start_key: &str, _count: usize) -> Vec<Vec<u8>> {
        // Redis has no ordered range scan on string keys
        Vec::new()
    }
}
...
fn print_comparison_table(results: &[WorkloadResult], engines: &[&str]) {
    ...
    for label in &workload_labels {
        eprint!("  {:<12}", format!("Workload {}", label.to_ascii_uppercase()));
        for engine in engines {
            let result = results
                .iter()
...
fn main() {
    ...
    for &label in &config.workloads {
        ...
        // LMDB
        {
            let mut db = LmdbYcsb::new(&tmpdir);
            eprint!("  lmdb...");
            let r = run_workload(&mut db, workload, config.records, config.ops);
            eprintln!(" {:.0} ops/s, p99={:.1}us", r.run_ops_sec, r.p99_us);
            all_results.push(r);
        ...
        // Redis (optional)
        if let Some(mut db) = RedisYcsb::new() {
            eprint!("  redis...");
            let r = run_workload(&mut db, workload, config.records, config.ops);
            eprintln!(" {:.0} ops/s, p99={:.1}us", r.run_ops_sec, r.p99_us);
            all_results.push(r);
    ...
    // Determine which engines participated
    engine_names = vec!["strata", "rocksdb", "lmdb", "sqlite"];
    if all_results.iter().any(|r| r.engine == "redis") {
        engine_names.push("redis");
    }
