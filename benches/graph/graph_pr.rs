//! LDBC Graphalytics PageRank Benchmark — Strata vs petgraph
//!
//! Fixed-iteration PageRank: PR(v) = (1-d)/|V| + d * Σ(PR(u)/deg(u))
//! Uses damping factor and iteration count from LDBC properties.
//!
//! Run:           `cargo bench --bench graph_pr`
//! Quick:         `cargo bench --bench graph_pr -- -q`
//! Custom data:   `cargo bench --bench graph_pr -- --dataset path/to/ldbc/dir`
...
fn record(
    ...
    params.insert("damping_factor".into(), serde_json::json!(damping));
    params.insert("num_iterations".into(), serde_json::json!(iterations));
...
fn main() {
    ...
    let damping = dataset.pr_damping_factor.unwrap_or(DEFAULT_DAMPING);
    let iterations = dataset.pr_num_iterations.unwrap_or(DEFAULT_ITERATIONS);
    let total = (dataset.vertices.len() + dataset.edges.len()) as f64;

    if !config.csv {
        eprintln!("=== LDBC Graphalytics PageRank Benchmark ===");
        eprintln!(
        ...
        eprintln!("Damping factor: {}", damping);
        eprintln!("PR iterations:  {}", iterations);
        eprintln!("Runs:           {}", config.runs);
        eprintln!();
    }

    let reference = if !config.no_validate {
        let ref_path = config.dataset.join(format!("{}-PR", dataset.name));
        if ref_path.exists() {
            Some(F64Reference::load(&ref_path).unwrap_or_else(|e| {
                eprintln!("Failed to load PR reference: {}", e);
                std::process::exit(1);
            }))
        } else {
            if !config.csv && !config.quiet {
                eprintln!("No PR reference file found, skipping validation.");
            }
            None
        }
    } else {
    ...
    let mut recorder = ResultRecorder::new("graph-pr");
    ...
    // --- Strata ---
    {
        ...
        for run in 0..config.runs {
            ...
            if run == 0 {
                ...
                if config.csv {
                    ...
                    let ms = times[0].as_secs_f64() * 1000.0;
                    let evps = total / times[0].as_secs_f64();
                    eprintln!("Strata PR: {:.3}ms, EVPS: {:.0}, |V|={}, |E|={}", ms, evps, dataset.vertices.len(), dataset.edges.len());
                }
            }
        }
        ...
        if !config.csv && !config.quiet {
            print_stats_table("Strata PageRank", &stats);
        }
        record(&mut recorder, "strata", &dataset, &stats, damping, iterations);
    }

    // --- petgraph ---
    {
        if !config.csv && !config.quiet {
            eprint!("Loading graph into petgraph (directed)...");
        }
        let pg_start = Instant::now();
        let (pg_graph, id_map) = dataset.to_petgraph_directed();
        ...
        for run in 0..config.runs {
            let start = Instant::now();
            let result = petgraph_pagerank(&pg_graph, damping, iterations);
            times.push(start.elapsed());
            ...
            if run == 0 {
                ...
                if config.csv {
                    ...
                    let ms = times[0].as_secs_f64() * 1000.0;
                    let evps = total / times[0].as_secs_f64();
                    eprintln!("petgraph PR: {:.3}ms, EVPS: {:.0}, |V|={}, |E|={}", ms, evps, dataset.vertices.len(), dataset.edges.len());
                }
            }
        }
        ...
        let stats = compute_stats(&mut times, total);
        if !config.csv && !config.quiet {
            print_stats_table("petgraph PageRank", &stats);
        }
        record(&mut recorder, "petgraph", &dataset, &stats, damping, iterations);
    }
