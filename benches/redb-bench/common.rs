...
pub struct RedisBenchDatabase {
    url: String,
}

impl RedisBenchDatabase {
    pub fn new(url: &str) -> Option<Self> {
        // Verify connection works before returning
        let client = redis::Client::open(url).ok()?;
        let mut con = client.get_connection().ok()?;
        let _: () = redis::cmd("FLUSHDB").query(&mut con).ok()?;
        Some(Self {
            url: url.to_string(),
        })
    }
...
impl BenchDatabase for RedisBenchDatabase {
    type C<'db>
...
pub struct RedisBenchDatabaseConnection {
    con: std::cell::RefCell<redis::Connection>,
}

// Redis connection is not Send by default, but we only use it single-threaded
// within each benchmark phase. The multi-threaded reads create new connections.
unsafe impl Send for RedisBenchDatabaseConnection {}
unsafe impl Sync for RedisBenchDatabaseConnection {}

impl BenchDatabaseConnection for RedisBenchDatabaseConnection {
    type W<'db>
        = RedisBenchWriteTransaction<'db>
    where
        Self: 'db;
    type R<'db>
        = RedisBenchReadTransaction<'db>
    where
        Self: 'db;

    fn write_transaction(&self) -> Self::W<'_> {
        RedisBenchWriteTransaction { con: &self.con }
    }

    fn read_transaction(&self) -> Self::R<'_> {
        RedisBenchReadTransaction { con: &self.con }
    }
}

pub struct RedisBenchWriteTransaction<'a> {
    con: &'a std::cell::RefCell<redis::Connection>,
}

impl<'a> BenchWriteTransaction for RedisBenchWriteTransaction<'a> {
    type W<'txn>
        = RedisBenchInserter<'txn>
    where
        Self: 'txn;

    fn get_inserter(&mut self) -> Self::W<'_> {
        RedisBenchInserter { con: self.con }
    }
...
pub struct RedisBenchInserter<'a> {
    con: &'a std::cell::RefCell<redis::Connection>,
}

impl BenchInserter for RedisBenchInserter<'_> {
    fn insert(&mut self, key: &[u8], value: &[u8]) -> Result<(), ()> {
        self.con
            .borrow_mut()
            .set::<_, _, ()>(key, value)
...
pub struct RedisBenchReadTransaction<'a> {
    con: &'a std::cell::RefCell<redis::Connection>,
}

impl<'a> BenchReadTransaction for RedisBenchReadTransaction<'a> {
    type T<'txn>
        = RedisBenchReader<'a>
    where
        Self: 'txn;
    ...
    fn get_reader(&self) -> Self::T<'_> {
        RedisBenchReader { con: self.con }
    }
}

pub struct RedisBenchReader<'a> {
    con: &'a std::cell::RefCell<redis::Connection>,
}

impl BenchReader for RedisBenchReader<'_> {
    type Output<'out>
        = Vec<u8>
    where
        Self: 'out;
    ...
    fn get(&self, key: &[u8]) -> Option<Vec<u8>> {
        self.con.borrow_mut().get(key).ok()
    }

    fn range_from<'a>(&'a self, _key: &'a [u8]) -> Self::Iterator<'a> {
        // Redis has no ordered range scan on plain keys.
        // Return empty iterator — this benchmark row will show degraded results.
        RedisBenchIterator {
    ...
    fn len(&self) -> u64 {
        redis::cmd("DBSIZE")
            .query::<u64>(&mut *self.con.borrow_mut())
            .unwrap_or(0)
    }
}

pub struct RedisBenchIterator {
    results: std::vec::IntoIter<(Vec<u8>, Vec<u8>)>,
}

impl BenchIterator for RedisBenchIterator {
    type Output<'out>
        = Vec<u8>
    where
        Self: 'out;
